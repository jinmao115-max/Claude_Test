...
"""
27卒 7社就職比較表 生成スクリプト（山陽色素除く）
- A4横1枚 PDF
- 元7社PDFと同じ行構成（営業拠点(参考)なし）
- ざっくり比較はセルを結合して横幅フル表示
- データ出典: マイナビ2027 / OpenWork / 公式採用サイト / kabutan(業績) / indeed / 日経会社情報
"""
import os, shutil
import openpyxl
...
TMP_PDF  = r"C:\Users\ka94654\AppData\Local\Temp\cmp7_measure_tmp.pdf"
OUT_LOCAL = r"C:\Users\ka94654\AppData\Local\Temp\27卒_7社就職比較表.pdf"
...
ROWS = [
    ...
    ("最新売上高", [
        ...
        "140億2,800万円(2026/3期)", "連結477億2,700万円(2026/3期)", "連結2,777億3,600万円(2026/3期)", "連結1,405億円(2025/3期)"]),
    ("最新営業利益", [
        "連結79億円(2025/3期)", "連結21.4億円(+72.6%/2025/3期)", "非公表",
        "8億2,400万円(2026/3期/+34.7%)", "営業60億1,200万円/経常61億9,100万円(2026/3期)", "連結273億2,000万円(2026/3期/+30.5%)", "66億6,000万円(2025/3期)"]),
    ("過去10年の業績トレンド", [
        "中長期で拡大・2026/3期は一時減益(ケミカル低迷)", "売上横ばい〜緩増、近年利益急回復", "350〜380億前後で緩やか成長・新工場建設中",
        "内航船需要安定・2026/3期増収増益(+34.7%)", "半導体向けリン酸で急成長・2026/3期も増収増益継続", "4年連続増収・電力インフラ/DC需要で急拡大(売上2,777億)", "2年連続増収(特装車需要堅調・過去最高更新)"]),
    ("株価推移(10年)", [
        "プライム・中長期上昇基調", "横ばい〜小幅上昇", "該当なし(非上場)",
        "スタンダード(6018)・横ばい〜緩増", "プライム(4022)・半導体需要で急上昇", "プライム(5805)・近年大幅上昇(電力/DC恩恵)", "プライム(7226)・特装車需要で堅調"]),
    ("", None),
    ("【勤務地・拠点(営業所除く)】", None),
    ("理系・技術の主な勤務地", [
    ...
    ("工場・研究拠点(例)", [
        "四日市研究所/工場、技術センター(伊勢原)", "八日市・鈴鹿・尾道・長野ほか", "東京工場(茨城)、大阪工場(堺)",
    ...
    ("27卒主な募集職種", [
        ...
        "技術系(エンジン設計・開発・製造)・営業系・事務系", "技術職(開発・製造・品質)・営業・管理(11〜15名)", "技術系総合職/エリア総合職(研究・生産・営業等)", "技術系(開発・設計・生産技術)・営業・管理系"]),
    ("", None),
    ("【OpenWork・働き方】", None),
    ("OpenWork総合評価(5点)", [
    ...
    ("平均年収(会社全体)", [
        ...
        "日経624万/OW非公表(9人)", "日経650万/OW506万(10人)", "日経729万/OW530万(36人)", "日経680万/OW541万(22人)"]),
    ("30歳前後の年収目安", [
    ...
    ("初任給(大卒・月)", [
        ...
        "234,000円(大卒・2025/4)※院了252,000円", "266,500円(大卒・手当込)※院了287,100円", "287,000円(大卒総合職)※院了309,000円", "273,000円(大卒)※院了288,000円"]),
    ("賞与実績(昨年・年間)", [
        "年2回・口コミ参考約5ヶ月(要確認)", "年2回・昨年実績約4ヶ月分", "年2回・7.2ヶ月分",
        "年2回(7月・12月)", "年2回・5.81ヶ月(2026年実績)", "年2回(6月・12月)", "年2回(6月・12月)"]),
    ("年間休日", [
        "125日前後(採用サイトFAQ)", "120日前後(本社)", "127日(公式HP)",
        "125日(公式HP)", "123日", "121日以上(公式HP)", "123日(公式HP)"]),
    ("住宅手当・社宅", [
    ...
    ("事業の軸", [
        ...
        "舶用ディーゼルエンジン・推進装置(内航船55%シェア)", "化成品(高純度リン酸/半導体向け)・機械・電子材料", "電線・ケーブル・光ファイバ・超電導・電力インフラ・DC向け", "特装車(ゴミ収集車・ダンプ等)・環境・パーキング"]),
    ("マイナビ2027", [
...
AXIS_ROWS = [
    ("【ざっくり比較（就活の軸）】", None),
    ("初任給（大卒・月給）",
     "伯東(29.3〜30.3万・手当込) ＞ SWCC(28.7万) ＞ 極東開発(27.3万) ＞ ラサ工業(26.65万) ＞ ヤマトプロテック(25.1万) ＞ ダイニック(23.7万) ＞ 阪神内燃(23.4万)"),
    ("資本金",
     "SWCC(242億) ＞ 極東開発(119億) ＞ ラサ工業(84億) ＞ 伯東(81億) ＞ ダイニック(57億) ＞ 阪神内燃・ヤマトプロテック(〜9億)"),
    ("会社規模・売上",
     "SWCC(2,777億) ＞ 伯東(1,831億) ＞ 極東開発(1,405億) ＞ ラサ工業(477億) ＞ ダイニック(441億) ＞ ヤマトプロテック(384億) ＞ 阪神内燃(140億)"),
    ("口コミ総合（OpenWork）",
     "伯東(3.38) ＞ 阪神内燃(3.05) ＞ ラサ工業(2.92) ＞ SWCC(2.89) ＞ ダイニック(2.70) ＞ 極東開発=ヤマトプロテック(2.51)"),
    ("残業の少なさ（マイナビ/公式実績）",
     "ラサ工業(7.6h) ＞ 伯東(7.3h) ＞ ダイニック(10h程度) ＞ ヤマトプロテック(18.7h) ＞ SWCC(20h以内) ＞ 阪神内燃(22.2h) ＞ 極東開発(26.6h)"),
    ("賞与（昨年実績・年間）",
     "ヤマトプロテック(7.2ヶ月) ＞ ラサ工業(5.81ヶ月/2026年実績) ＞ 伯東・ダイニック(4〜5ヶ月前後) ／ SWCC・極東開発・阪神内燃は年2回(実績月数要確認)"),
    ("理系・化学系の専門性",
     "ラサ工業(半導体向けリン酸・化学急成長)／伯東(商社×化学)／ダイニック(コーティング)／SWCC(電線・超電導・DC向け急拡大)／極東開発(特装車・機械)／ヤマト(防災)／阪神内燃(舶用エンジン)"),
    ("社会貢献・インフラ感",
     "SWCC(電力インフラ・DC)・極東開発(廃棄物処理・特装)・ヤマトプロテック(防災)・阪神内燃(海運インフラ) ＞ 伯東・ラサ工業(半導体) ＞ ダイニック"),
    ("上場・将来性（株価）",
     "プライム: 伯東・ラサ工業(半導体需要◎)・SWCC(電力/DC急成長◎)・極東開発 ／ スタンダード: ダイニック・阪神内燃 ／ 非上場: ヤマトプロテック"),
    ("", None),
    ("出典: マイナビ2027 / OpenWork / 公式採用HP / kabutan・irbank(業績) / indeed / 日経会社情報。2026年6月時点。",
     "賞与・初任給の一部はユーザー確認値を反映。山陽色素は今回リストより除外。"),
]
